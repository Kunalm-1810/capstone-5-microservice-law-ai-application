...
function App() {
  ...
  const [history, setHistory] = useState({ chats: [], reviews: [] });
  const [historyTab, setHistoryTab] = useState("chats");
  ...
  const loadHistory = async () => {
    const [chats, reviews] = await Promise.all([
      axios.get("/history/chats"),
      axios.get("/history/reviews"),
    ]);
    setHistory({ chats: chats.data, reviews: reviews.data });
  };
      ...
      {tab === "history" && (
        <div className="history-section">
          <div className="history-tabs">
            <button className={historyTab === "chats" ? "active" : ""} onClick={() => setHistoryTab("chats")}>💬 Chat History</button>
            <button className={historyTab === "reviews" ? "active" : ""} onClick={() => setHistoryTab("reviews")}>📄 Contract Reviews</button>
          </div>

          {historyTab === "chats" && (
            <div className="history-list">
              {history.chats.length === 0 && <p className="no-history">No chat history yet.</p>}
              {history.chats.map((c) => (
                <div key={c._id} className="history-card">
                  <div className="history-q">👤 {c.userMessage}</div>
                  <div className="history-a">⚖️ {c.botReply}</div>
                  <div className="history-date">{new Date(c.createdAt).toLocaleString()}</div>
                </div>
              ))}
            </div>
          )}
          ...
          {historyTab === "reviews" && (
            <div className="history-list">
              {history.reviews.length === 0 && <p className="no-history">No contract reviews yet.</p>}
              {history.reviews.map((r) => (
                <div key={r._id} className="history-card">
                  <div className="history-q">📄 {r.contractText.slice(0, 120)}...</div>
                  <div className="history-a">📋 {r.reviewResult}</div>
